...
saveaddress = dir+'\\traindata\\train_model\\' + str(n_episode) + taddr
saveaddress_raw = saveaddress+'\\raw'

loss_saveaddress = dir + '\\traindata\\losses_average\\' + str(n_episode) + taddr
...
def train(episode, trainrender=False, LR = 0.0005, GAMMA = 0.99, TAU = 1e-3):
    trainagent = Agent(env.observation_space.shape[0], env.action_space.n,  device, BATCH_SIZE, BUFFER_SIZE, LR, GAMMA, TAU)
    ...
    for i in range(episode):
        ...
        while True:
            ...
            if eps >= eps_end:
                eps *= eps_iteration
            action = trainagent.act(state, eps)  #单个的数

            next_state, reward, done, _ = env.step(action)

            loss = trainagent.step(state, action, reward, next_state, done)
...
#                print('\rSteps: {}, loss average: {}'.format(steps, loss))
            ...
            if done:
#                print('\repisode: {}, step:{}, state: {}, action: {}, reward: {}, score: {}'.format(episode, step, state, action, reward, score), end='')
                break
        print('\rTRAIN: episode: {}, steps: {}, score: {}'.format(i, steps, score), end=' ')
        ...
        if i % c01 == 0 and i >= 0:
            torch.save(trainagent.Qnet_eval.state_dict(), dir+'\\traindata\\load\\DQN_normal_train' + '_' + str(i) + '.pth')
            if DEBUG == False:
                ...
                torch.save(trainagent.Qnet_eval.state_dict(), saveaddress_raw + '\\DQN_normal_train_' + 'LR_{}, GAMMA_{}, TAU_{}'.format(LR, GAMMA, TAU) + '_ ' + str(i) + '_ ' + str(score_average) + '.pth')

        if DEBUG == False:
            if score_average >= old_score_average:
                torch.save(trainagent.Qnet_eval.state_dict(), saveaddress+'\\DQN_normal_best_' + str(n_episode) + '_LR_{}, GAMMA_{}, TAU_{}'.format(LR, GAMMA, TAU) + '_.pth')
...
#        plt.show()
    ...
    if DEBUG == False:
        if os.path.exists(loss_saveaddress) is False:
            os.mkdir(loss_saveaddress)
        np.savez(loss_saveaddress + '\\DQN_train_losses_average_' + 'LR_{}, GAMMA_{}, TAU_{}_'.format(LR, GAMMA, TAU) + '.npz',
        ...
        np.savez(score_saveaddress + '\\DQN_train_scores_average_' + 'LR_{}, GAMMA_{}, TAU_{}_'.format(LR, GAMMA, TAU) + '.npz',
                 data1=np.arange(len(scores_average)), data2=scores_average)


def test(episode, testrender=False, LR = 0., GAMMA = 0., TAU = 0.):
    testagent = Agent(env.observation_space.shape[0], env.action_space.n,  device, BATCH_SIZE, BUFFER_SIZE)
    testscores_average = []
    for j in range(int((n_episode / c01)+1)):
        testagent.Qnet_eval.load_state_dict(torch.load(dir+'\\traindata\\load\\DQN_normal_train' + '_' + str(j*c01) + '.pth'))
...
#        plt.show()
    ...
    if DEBUG == False:
        ...
        np.savez(test_saveaddress + '\\DQN_test_testscores_average_' + str(n_episode) + '_LR_{}, GAMMA_{}, TAU_{}_'.format(LR, GAMMA, TAU) + '.npz',
    ...
    #    plt.savefig(dir+'\\image\\Normal'+time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime())+'.png')
        plt.savefig(image_saveaddress + '\\DQN_' + str(n_episode) + '_LR_{}, GAMMA_{}, TAU_{}'.format(LR, GAMMA, TAU) + '_.png')
    plt.close()
    env.close()
...
if __name__ == '__main__':
    ...
    if DEBUG == True:
        ...
        for ilr in [0.0001, 0.0005, 0.01, 0.05, 0.1, 0.5, 1]:
    ...
    # P_LR = 0.0005 # P_GAMMA = 0.99 # P_TAU = 1e-3