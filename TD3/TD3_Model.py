...
class Actor(nn.Module):
    def __init__(self, args, log_std_min=-20, log_std_max=2):
        ...
        setattr(self, 'output', nn.Linear(args.net_dim, args.action_dim))
        layer_init(eval("self." + 'output'))
    ...
    def forward(self, state, valid=None):
        ...
        if self.action_type == 1:
            output = getattr(self, 'output')(x)
            action = torch.tanh(output) * self.max_action
            return action
